...
target_network = sys.argv[2] if len(sys.argv) > 2 else None
network_env_file = {
    "localnet": ".env.localnet",
    "testnet": ".env.testnet",
    "mainnet": ".env.mainnet",
}.get(target_network or "")

if network_env_file and (root_env_file := Path.cwd() / network_env_file).exists():
    logger.info("Loading %s", network_env_file)
